export function calculateJulianDate(currentDate: Date): number {
...
export function AdjustSolarRadiation(totalRadiationAverage: number) {
...
export function CalcLiquidRadiation(
  ...
  currentSteamPressureValue: number,
  nebulosityFactor: number
) {
...
export function CalcSunsetHourAngle(phi: number, solarDeclination: number) {
...
export function CalcGAsterico(gama: number, windVelocity: number) {
  //Fator da planilha
  // 200/208 = 0.9615
  // 50/208 = 0.24
...
export function CalcSteamPressureEstimation(temperatureMin: number) {
...
export function CalcGama(atmosphericPressure: number) {
  // Gama pelo documento
...
export function CalcAero(
  ...
  delta: number,
  g_asterico: number,
  atmosphericTemperatureAverage: number,
  windVelocity: number,
  saturationSteamPressure: number,
  currentSteamPressureValue: number
) {
  return (
    (gama *
