import { UserType, UserTypes } from "../../../../../modules/user/core/model/user";
import {
  Modules,
  SystemModulesProps,
...
import { BaseUserModel } from "../../../../../modules/user/core/model/base-user";
...
export class DbAccountRepository implements AccountRepositoryProtocol {
  ...
  async getUserByCode(code: string): Promise<BaseUserModel | null> {
  ...
  async getUserById(id_user: number): Promise<BaseUserModel | null> {
    const result = await governmentDb
  ...
  async getByEmail(email: string): Promise<BaseUserModel | null> {
    const result = await governmentDb
      .select("*")
      .from("User")
      .where("Email", email)
  ...
  async getByLogin(login: string): Promise<BaseUserModel | null> {
    const result = await governmentDb
      .select("*")
      .from("User")
      .where("Login", login)
  ...
  async getById(id_user: number): Promise<Required<BaseUserModel> | null> {
    const result = await governmentDb
      .select("*")
      .from("User")
      .where({ Id: id_user })
    ...
    const user: Required<BaseUserModel> = {
      id: Id,
      name: Name,
      login: Login,
      email: Email,
